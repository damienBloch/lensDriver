...
scpi_error_t SCPI::get_temperature_correction_status(struct scpi_parser_context* context, struct scpi_token* command)
{
  ...
  if(DAC::temperatureCorrection)
    Communication::StreamInterface->println("Enabled");
  else Communication::StreamInterface->println("Disabled");
  return SCPI_SUCCESS;
}
...
scpi_error_t SCPI::get_filterTime(struct scpi_parser_context* context, struct scpi_token* command)
{
  ...
  Communication::StreamInterface->println(configuration.intensityCorrection.filterTime);
  return SCPI_SUCCESS;
}
...
scpi_error_t SCPI::get_intensityToCurrent(struct scpi_parser_context* context, struct scpi_token* command)
{
  ...
  Communication::StreamInterface->println(configuration.intensityCorrection.intensity2current);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);  
  Communication::StreamInterface->println(configuration.intensityCorrection.volt2intensity);
  return SCPI_SUCCESS;
}
...
scpi_error_t SCPI::get_intensity(struct scpi_parser_context* context, struct scpi_token* command)
{
  Timer1.stop();
  scpi_free_tokens(command);  
  Communication::StreamInterface->println(A2DC::codeToIntensity(A2DC::readValue),4);
  Timer1.start();
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.temperatureCorrection.temperature2current,4);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(A2DC::_filteredIntensity,4);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println("Lens driver");
  return SCPI_SUCCESS;
}

scpi_error_t SCPI::measure_temperature(struct scpi_parser_context* context, struct scpi_token* command)
{
  if(LENS::temperature >0 & LENS::temperature<100)Communication::StreamInterface->println(LENS::temperature,4);  
  else
  {
    scpi_error error;
    error.id = -201;
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.TEC.P);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  if(DAC::mode==CONSTANT)Communication::StreamInterface->println("Constant");
  if(DAC::mode==ARBITRARY)Communication::StreamInterface->println("Arbitrary");
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.TEC.imin);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.TEC.imax);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.DAC.frequency);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.DAC.imax);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.DAC.imin);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.TEC.I);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.TEC.D);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(configuration.TEC.setpoint);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(TEC::output);
  return SCPI_SUCCESS;
}
...
{
  scpi_free_tokens(command);
  Communication::StreamInterface->println(DAC::current);
  return SCPI_SUCCESS;
}
...
scpi_error_t SCPI::get_sequence(struct scpi_parser_context* context, struct scpi_token* command)
{
  if(DAC::len>0)
  {
    ...
    {
      Communication::StreamInterface->print(DAC::codeToCurrent(DAC::getWordAt(i)),3);
      Communication::StreamInterface->print(" ");
    }
    Communication::StreamInterface->println();
  }
  else
  {
    scpi_error error;
...
scpi_error_t SCPI::set_sequence(struct scpi_parser_context* context, struct scpi_token* command)
{
  ...
  if(output_numeric.length == 0)
  {
    ...
    if(DAC::arr!=NULL)
    {
      ...
      for(int i=0;i<N;i++)
      {
          String valueStr=Communication::StreamInterface->readStringUntil('\n');
